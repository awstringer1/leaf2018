...
$$
This just makes sense; of course the best guess of the value of the *probability* of heads based on one sample is just the relative frequency with which $X$ is heads in that sample.
...
For our example, we will choose a Beta(12,12) distribution, for reasons we will discuss below in the section labelled **conjugate priors**. This looks like this:
```{r betaprior-vis2}
...
This prior puts strong weight on the coin being close to fair; values below $\theta = 0.3$ and $\theta = 0.7$ have very little prior probability. This can be verified exactly:
```{r betaprior-prob1}
...
$$
interpreted as a weighted average of possible values of $\theta$, with weights corresponding to their probabilities (densities); or, we may use the *posterior mode*:
$$
...
## Non-conjugate Priors & Posterior Sampling
